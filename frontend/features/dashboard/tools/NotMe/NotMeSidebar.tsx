...
export const NotMeSidebar: React.FC = () => {
  const { listItems, addItem, removeItem } = useNotMe();
    ...
    <div className="h-full w-full flex flex-col bg-black border-l border-white/5 p-4">
      ...
      <div className="flex-1 overflow-y-auto custom-scroll space-y-2">
        {listItems.map((item) => (
          <div
            key={item.id}
            className="group flex items-center justify-between p-3 bg-white/5 rounded-lg hover:bg-white/10 transition-all border border-transparent hover:border-white/5"
          >
            <div className="flex items-center gap-3">
              {/* Extremely simple icon rendering for now */}
