    ...
    <div className="fixed inset-0 z-[60] bg-[#1e1e1e] text-[#cccccc] flex flex-col font-sans select-none overflow-hidden">
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-12 bg-[#333333] flex flex-col items-center py-4 space-y-4">
          <button 
            ...
            className={`p-2 transition-colors ${activeTool === 'MAGNETIC' ? 'text-white border-l-2 border-orange-500' : 'text-[#858585] hover:text-white'}`}
          >
          ...
          <button 
            ...
            className={`p-2 transition-colors ${activeTool === 'LUCKY_GIRL_SYNDROME' ? 'text-white border-l-2 border-orange-500' : 'text-[#858585] hover:text-white'}`}
          >
          ...
          <button 
            ...
            className={`p-2 transition-colors ${activeTool === 'MIDNIGHT_FICTION' ? 'text-white border-l-2 border-orange-500' : 'text-[#858585] hover:text-white'}`}
          >
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
          </button>
          <div className="flex-1"></div>
          <button onClick={onExit} className="p-2 text-[#858585] hover:text-white" title="Exit Dev Mode">
        ...
        <div className="w-64 bg-[#252526] border-r border-black/20 flex flex-col">
          <div className="p-3 text-[11px] font-bold text-[#bbbbbb] tracking-wider uppercase">Explorer</div>
          <div className="flex-1 overflow-y-auto px-2 space-y-1">
            <div className="p-1 px-2 text-xs hover:bg-[#37373d] rounded cursor-pointer flex items-center group">
              <span className="text-orange-500 mr-2">▼</span>
              <span className="font-semibold">{activeTool.replace(/_/g, ' ')}</span>
            </div>
            {activeTool === 'MAGNETIC' && tasks.map(t => (
              <div key={t.id} className="p-1 px-6 text-xs text-[#858585] hover:text-white truncate">
                {t.completed ? '✓ ' : '○ '} {t.text}
              </div>
            ))}
            {activeTool === 'LUCKY_GIRL_SYNDROME' && schedule.map(s => (
              <div key={s.id} className="p-1 px-6 text-xs text-[#858585] hover:text-white truncate">
                [{s.time}] {s.activity}
              </div>
            ))}
          </div>
          ...
          <div className="flex-1 p-10 flex flex-col">
            {activeTool === 'MAGNETIC' && (
              <div className="max-w-xl">
                <h1 className="text-3xl font-bold mb-8 text-white">Magnetic / Task Management</h1>
                <div className="space-y-4">
                  {tasks.map(task => (
                    <div key={task.id} className="flex items-center p-3 bg-[#252526] hover:bg-[#2a2d2e] rounded border border-white/5 group cursor-pointer" onClick={() => toggleTask(task.id)}>
                      <div className={`w-5 h-5 rounded border-2 mr-4 flex items-center justify-center transition-colors ${task.completed ? 'bg-orange-500 border-orange-500' : 'border-[#444444]'}`}>
                        {task.completed && <span className="text-black font-bold">✓</span>}
                      </div>
                      <span className={`${task.completed ? 'line-through text-neutral-600' : 'text-[#cccccc]'}`}>{task.text}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {activeTool === 'LUCKY_GIRL_SYNDROME' && (
              <div className="max-w-xl">
                <h1 className="text-3xl font-bold mb-8 text-white">Lucky Girl Syndrome / Schedule</h1>
                <div className="space-y-2 border-l border-[#333333] ml-4">
                  {schedule.map(item => (
                    <div key={item.id} className="relative pl-8 py-4 group">
                      ...
                      <div className="text-sm font-mono text-orange-500 mb-1">{item.time}</div>
                      <div className="text-lg text-white font-semibold">{item.activity}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {activeTool === 'MIDNIGHT_FICTION' && (
              <div className="flex-1 flex flex-col items-center justify-center text-center">
                <h1 className="text-2xl font-bold mb-4 text-[#858585] uppercase tracking-[0.2em]">Midnight Fiction / Focus</h1>
      ...
      <div className="h-6 bg-orange-600 text-black text-[11px] flex items-center px-4 justify-between font-medium">
        <div className="flex items-center space-x-4">
          <div className="flex items-center"><span className="mr-1">⎇</span> dev-branch</div>
          <div className="flex items-center"><span className="mr-1">⚠</span> 0</div>
        </div>
        <div className="flex items-center space-x-4">
          <div className="uppercase tracking-tighter">VON.CORE CORE-ENGINE-1.0</div>
          <div>UTF-8</div>
